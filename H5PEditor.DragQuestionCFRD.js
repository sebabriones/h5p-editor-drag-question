 ...
 * @param {jQuery} $
 */
H5PEditor.widgets.dragQuestion = H5PEditor.DragQuestionCFRD = (function ($, DragNBar) {
  /**
  ...
  C.prototype.updateDropZone = function (dropZone, id) {
    ...
    if (params.tipsAndFeedback !== undefined && params.tipsAndFeedback.tip !== undefined && params.tipsAndFeedback.tip.trim().length !== 0) {
      dropZone.$dropZone.append(H5P.JoubelUICFRD.createTip(params.tipsAndFeedback.tip, {showSpeechBubble: false}));
    }
    ...
    var $inner = dropZone.$dropZone.children('.h5p-inner');

    if ($inner.length) {
      C.setOpacity($inner, 'background', params.backgroundOpacity);
    }
    else {
  ...
  C.prototype.refreshAppearanceStyles = function () {
    ...
    for (i = 0; i < this.dropZones.length; i++) {
      ...
      $inner = dropZone.$dropZone.children('.h5p-inner');
